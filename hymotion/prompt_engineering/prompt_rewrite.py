...
class PromptRewriter:
    def __init__(
        ...
        quantization: str = "nf4",  # "nf4", "int4", "fp16"
    ...
    def _load_model(self):
        ...
        print(f"[HY-Motion] Loading Text2MotionPrompter ({self.quantization}, double_quant={self.double_quant})")
        
        # Determine ComfyUI models directory for Text2MotionPrompter
        try:
            import folder_paths
            comfy_models_dir = folder_paths.models_dir
        except ImportError:
            # Fallback if folder_paths not available
            comfy_models_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "models")
        
        # Target directory: ComfyUI/models/hymotion/Text2MotionPrompter
        target_dir = os.path.join(comfy_models_dir, "hymotion", "Text2MotionPrompter")
        ...
        if os.path.exists(config_file):
            ...
            print(f"[HY-Motion] Using local model: {target_dir}")
            model_load_path = target_dir
        else:
            # Need to download
            print(f"[HY-Motion] Model not found in {target_dir}")
            print(f"[HY-Motion] Downloading Text2MotionPrompter to ComfyUI models directory...")
            print(f"[HY-Motion] This is a one-time download (~4GB)")
            ...
            try:
                ...
                print(f"[HY-Motion] Download complete: {local_path}")
                model_load_path = local_path
            except Exception as e:
                print(f"[HY-Motion] Download failed: {e}")
                print(f"[HY-Motion] Falling back to HuggingFace cache...")
                model_load_path = self.model_path
        
        # Load tokenizer first (lightweight)
        print("[HY-Motion] Loading tokenizer...")
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_load_path,
            trust_remote_code=True
        )
        
        # Load model with user-selected quantization
        print("[HY-Motion] Loading model weights...")
        if self.quantization == "fp16":
            # No quantization - pure float16
            print("[HY-Motion] Using FP16 (no quantization)")
            ...
            try:
                ...
                print("[HY-Motion] bitsandbytes not available, falling back to FP16")
                self.model = AutoModelForCausalLM.from_pretrained(
                    model_load_path,
                    torch_dtype=torch.float16,
                    device_map="auto",
        ...
        print("[HY-Motion] Text2MotionPrompter loaded successfully")
