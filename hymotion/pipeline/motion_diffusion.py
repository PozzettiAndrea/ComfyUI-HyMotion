...
class MotionGeneration(torch.nn.Module):
    ...
    def _decode_o6dp(
        ...
        if self.body_model is not None:
            ...
            with torch.no_grad():
                # Use batched forward for efficiency (processes in chunks)
                batch_params = {
                    "rot6d": rot6d_smooth.unsqueeze(0) if rot6d_smooth.dim() == 3 else rot6d_smooth,
                    "trans": transl_smooth.unsqueeze(0) if transl_smooth.dim() == 2 else transl_smooth,
                }
                # Reshape to (B, T, J, 6) and (B, T, 3) for forward_batch
                if batch_params["rot6d"].dim() == 3:
                    batch_params["rot6d"] = batch_params["rot6d"].unsqueeze(0)
                    batch_params["trans"] = batch_params["trans"].unsqueeze(0)
                
                result = self.body_model.forward_batch(batch_params, chunk_size=64)
                vertices = result["vertices"].to(device)
                k3d = result["keypoints3d"].to(device)
